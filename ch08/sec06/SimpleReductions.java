...
public class SimpleReductions {
	public static void main(String[] args) throws IOException {
		...
		boolean aWordStartsWithQ = words.stream().anyMatch(s -> s.startsWith("QQ"));
...
//        words.stream().parallel().filter(s -> s.startsWith("Q")).distinct().peek(System.out::println).count();
		words.stream().parallel().filter(s -> s.startsWith("Q")).distinct().forEach(System.out::println);
	}
}
